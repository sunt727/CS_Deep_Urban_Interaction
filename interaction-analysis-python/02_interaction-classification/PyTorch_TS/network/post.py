...
def plot_pose(img_orig, joint_list, person_to_joint_assoc, bool_fast_plot=True, plot_ear_to_shoulder=False):
    ...
    to_box = canvas.copy() if bool_fast_plot else cv2.addWeighted(
        img_orig, 0.3, canvas, 0.7, 0)
    ...
    for person_joint_info in person_to_joint_assoc:
        ...
        for limb_type in range(which_limbs_to_plot):
            ...
            a_person_joint_coords += joint_coords.astype(int).tolist()
        ...
        a_person_joint_coords = np.array(a_person_joint_coords)
        bottom,right = tuple(a_person_joint_coords.max(0).astype(
            int))
        top,left = tuple(a_person_joint_coords.min(0).astype(
            int))
        
        f_top = int(max(top - (bottom - top) * 0.05,0))
        f_bottom = int(min(bottom + (bottom - top) * 0.05,w))
        f_left = int(max(left - (right - left) * 0.1,0))
        f_right = int(min(right + (right - left) * 0.1,h))

        # draw a rectangle as the boundary
        cv2.rectangle(to_box,(f_bottom, f_right),(f_top, f_left),(0,255,0),3)
        
        all_predicts.append((f_bottom, f_right, f_top, f_left))

    return to_plot, canvas, to_box, all_predicts
...
def decode_pose(img_orig, param, heatmaps, pafs):
    ...
    # (Step 4): plot results
    to_plot, canvas, to_box, all_predicts = plot_pose(img_orig, joint_list, person_to_joint_assoc)

    return to_plot, canvas, joint_list, person_to_joint_assoc, to_box, all_predicts
